...
import { setSolvedproblems } from '../store/solvedProblemSlice.js'
...
export default function UserProfile_2() {
    ...
    const { SolvedProblems } = useSelector((state)=>state.solvedproblem)
    ...
    useEffect(()=>{
        async function fetchdata(){
            ...
            const response = await axios.post(backend_url+"/problems/fetchdata_user",{
                username
            })
        ...
        if(SolvedProblems.length === 0) fetchSolvedProblems();
        console.log(SolvedProblems)
        fetchdata();
        // dispatch(setSolvedproblems(solvedproblems))
    },[])
    ...
    async function fetchSolvedProblems() {
        ...
        const response = await axios.get(backend_url+`/problems/getsolvedproblems?username=${username}`, {
            username: username
        });
        console.log(response.data)
        localStorage.setItem("solved_problems",response.data.length)
